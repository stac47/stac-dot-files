...
***** =libgccjit= not working

With =gcc= and =libgccjit= installed by =brew= and up-to-date I have
...
#+begin_quote
LD_LIBRARY_PATH="$(brew --prefix libgccjit)/lib/gcc/current/:$LD_LIBRARY_PATH" CPPFLAGS="-I$(brew --prefix libgccjit)/include" ./configure
#+end_quote

Removes the warning but we fail anyway with the usage of
=libgccjit=. For the logs, we can see:
...
If the compilation fails, we can try the =bootstrap= make task.

#+begin_src bash
  make -j${JOBS} NATIVE_FULL_AOT=1 bootstrap
#+end_src

If that did not solve the issue, try to clean your workspace with:
