...
Until Emacs 30 is out, we need a package to be able to use
=package-vc-install= with =use-package=. This allows to install
package directly from source rather than for a package repository.
