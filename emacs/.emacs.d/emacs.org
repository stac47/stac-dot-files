...
*** Automatic Pairs Completion

When writing code or text, I find it convenient Emacs automatically
adds the associated closing character.
...
The following package integrates the =hadolint= =Dockerfile=
linter. This one must be installed on the system to have =flymake=
interactively linting =Dockerfile=s:
...
Here is a collection of some snippets to evaluate if I can get rid of
yasnipet.
...
Here are the packages dedicated to languages I use.
...
=kubed= is an amazing package that uses transcients to navigate into
and manipulate an =kubernetes= clusters.
